...
max_ds=`eval "bq --bigqueryrc /root/.bigqueryrc/bigquery.rc query --use_legacy_sql=false --quiet --format=csv \"SELECT MAX(schema_name) FROM clingen-stage.INFORMATION_SCHEMA.SCHEMATA WHERE REGEXP_CONTAINS(schema_name, r'clinvar_[0-9]{4}_[0-9]{2}_[0-9]{2}')\" | awk '{if(NR>1)print}'"`
...
echo "$x" | while read id name ; do
    ...
    # echo "bq --bigqueryrc /root/.bigqueryrc/bigquery.rc query --use_legacy_sql=false  'CALL \`clingen-stage.clinvar_qa.create_clinvar_dataset\`("\""${dataProject}.${name}"\"", "\""clingen-stage.${name}"\"")'"
    eval "bq --bigqueryrc /root/.bigqueryrc/bigquery.rc query --use_legacy_sql=false  'CALL \`clingen-stage.clinvar_qa.create_clinvar_dataset\`("\""${dataProject}.${name}"\"", "\""clingen-stage.${name}"\"")'"
