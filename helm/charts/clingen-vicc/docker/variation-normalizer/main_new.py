...
def vrs_python_translate_from(
    ...
    fmt: Optional[TranslateFromFormat] = Query(
        None, description=from_fmt_descr),
) -> TranslateFromService:
    ...
    try:
        ...
        warnings.append(
            f"vrs-python translator raised {type(e).__name__}: {e}")
...
def amplification_to_cx_var(
    ...
    sequence_id: Optional[str] = Query(
        None, description="Sequence identifier"),
    start: Optional[int] = Query(
        None, description="Start position as residue coordinate"
    ),
    end: Optional[int] = Query(
        None, description="End position as residue coordinate"),
    untranslatable_returns_text: bool = Query(
        False, description=untranslatable_descr),
) -> AmplificationToCxVarService:
